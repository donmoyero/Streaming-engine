...
function animateRoomLights(delta) {
  ...
  // Monitor glow gentle pulse (only when not overridden by activity)
...
// We track where she is in world space and which way she faces.
const vrmWorld = { x: 0, y: 0, z: 0, facingY: 0 }; // facingY = world Y-rotation she faces

// ── VRM Walking / Waypoint System ────────────────────────
// She walks to named locations in the room. Her body rotates
// to face the target. The camera orbits to stay in front.
//
// Waypoints are in world space (x, z). y is always floor level.
// ─────────────────────────────────────────────────────────
...
const walk = {
  ...
  toX:       0, toZ: 0,
  progress:  0,     // 0→1
  duration:  2.0,   // seconds to walk between points
  targetFacing: 0,  // world Y rotation she faces at destination
  onArrive:  null,  // callback when she arrives
};

// Current world position (separate from vrm.scene.position which is in model-space centre)
...
function walkTo(waypointName, onArrive = null) {
  ...
  // Face toward target — camera will swing to stay in front
  walk.targetFacing = Math.atan2(dx, dz) + Math.PI; // +PI so she faces the direction of travel
}
...
function updateWalk(delta) {
  ...
  // Apply to VRM scene (keeping Y for jumps etc.)
  vrm.scene.position.x = vrmPos.x;
  vrm.scene.position.z = vrmPos.z;

  // Rotate VRM to face direction of travel
  // Lerp facing rotation smoothly
...
// ── Room Roaming — automatic waypoint patrol ──────────────
// She wanders between locations based on her current activity.
// Activity destinations map activities to preferred waypoints.
