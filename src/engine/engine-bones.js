...
export function activityUpdate(delta) {
  ...
  switch (ACTIVITY.current) {
    ...
    case 'fireGaze': {
      ...
      setExpression(flicker > 1.1 ? 'happy' : flicker > 0.5 ? 'neutral' : 'sad');
...
export function doBlink() {
  ...
  if (r < 0.10) {
    // Double blink
    setBS('blink_l', 1); setBS('blink_r', 1);
    setTimeout(() => { setBS('blink_l', 0); setBS('blink_r', 0); }, 110);
    setTimeout(() => { setBS('blink_l', 1); setBS('blink_r', 1); }, 220);
    setTimeout(() => { setBS('blink_l', 0); setBS('blink_r', 0); }, 340);
  } else if (r < 0.22) {
    // Slow blink — dreamy, held longer
    setBS('blink_l', 1); setBS('blink_r', 1);
    setTimeout(() => { setBS('blink_l', 0); setBS('blink_r', 0); }, 260);
  } else if (r < 0.35) {
    // Slightly asymmetric — right eye a beat behind
    setBS('blink_l', 1);
    setTimeout(() => setBS('blink_r', 1), 28);
    setTimeout(() => setBS('blink_l', 0), 130);
    setTimeout(() => setBS('blink_r', 0), 155);
  } else {
    // Normal blink
    setBS('blink', 1);
...
export function doBlinkMr() {
  const r = Math.random();
  if (r < 0.10) {
    setBSMr('blink_l', 1); setBSMr('blink_r', 1);
    setTimeout(() => { setBSMr('blink_l', 0); setBSMr('blink_r', 0); }, 110);
    setTimeout(() => { setBSMr('blink_l', 1); setBSMr('blink_r', 1); }, 240);
    setTimeout(() => { setBSMr('blink_l', 0); setBSMr('blink_r', 0); }, 360);
  } else if (r < 0.22) {
    setBSMr('blink_l', 1); setBSMr('blink_r', 1);
    setTimeout(() => { setBSMr('blink_l', 0); setBSMr('blink_r', 0); }, 250);
  } else if (r < 0.35) {
    setBSMr('blink_r', 1);
    setTimeout(() => setBSMr('blink_l', 1), 30);
    setTimeout(() => setBSMr('blink_r', 0), 125);
    setTimeout(() => setBSMr('blink_l', 0), 150);
  } else {
    setBSMr('blink', 1);
