...
async function _turn(who, text, mood = 'happy') {
  ...
  // Full mood → expression map — uses all available blendshapes
  const MOOD_EXPR = {
    laugh:  'excited',
    hype:   'excited',
    slay:   'happy',
  ...
  const expr = MOOD_EXPR[mood] || 'happy';

  if (who === 'lora') {
    showLoraBubble(text);
    setExpressionMr(expr);
    floatEmoji('lora', emojiStr, emojiCount);
    // Miss reacts to Lora speaking
    if (window._triggerMissReaction) window._triggerMissReaction(mood);
